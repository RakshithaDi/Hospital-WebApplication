<%-- 
    Document   : feedback
    Created on : May 8, 2021, 11:29:22 PM
    Author     : PC
--%>
...
<body style="background: #dddddd;">
    ...
    <nav class="navbar navbar-light navbar-expand-md d-flex d-xl-flex align-items-center align-content-center align-self-center mx-auto justify-content-xl-center align-items-xl-center navigation-clean-search">
        <div class="container"><a class="navbar-brand" href="#"><i class="fas fa-hospital" style="font-size: 25px;"></i></a><button data-toggle="collapse" class="navbar-toggler" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
            ...
            <div class="dropdown menu_links"><a class="dropdown-toggle" aria-expanded="false" data-toggle="dropdown" style="margin-right: 10px;color: rgb(0,0,0,0.75);border-color: rgba(0,0,0,0);"><%out.println("Admin ("+username+")");%> </a>
                <div class="dropdown-menu"><a class="dropdown-item" href="../../Logout">Logout</a></div>
            </div>
        </div>
    </nav>
    <div class="container" style="padding-right: 0px;padding-left: 0px;height: 844px;">
        <div class="col">
            <div class="card" style="margin: 11px;">
                <div class="card-body">  <%List<Feedback> list = DBConnection.getFeedback(); %>
            <%for(Feedback f1:list){ %>
                    <h6 class="card-title">feedback ID:  <%=f1.getFeedbckid()%> </h6>
                    <h6 class="text-muted card-subtitle mb-2">Rating:  <%=f1.getStars()%> stars</h6>
                    <p class="card-text"> <%=f1.getFeedbck()%>.</p>
                    <hr>
                 <%}%></div>
            </div>
        </div>
